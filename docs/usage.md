# DojoAgents Usage Notes

This document contains usage details moved out of the root `README.md`. Keep
the root README focused on basic dashboard startup only.
...
The Dashboard provides a React SPA frontend with an OpenAI-compatible chat API,
SSE streaming, and a Canvas panel for dynamic chart rendering.
...
gateway event plus a `ChatRequest` shape. `/api/send/{platform}/{target}` sends
a text message through the selected adapter.
...
1. Select from preset providers (`openai`, `anthropic`, `gemini`, `deepseek`) or define a `custom` endpoint, such as Ollama, llama.cpp, or vLLM.
