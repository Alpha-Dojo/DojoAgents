...
import { fetchCoreTickerIncome } from '../api/dojoCore';
import { cacheKeys } from '../cache/cacheKeys';
import { fetchCached, getCached } from '../cache/queryCache';
import type { CoreTickerContext } from '../navigation/coreContext';
import type { CoreTickerIncomeResponse } from '../types/dojoCore';

interface CoreStockIncomeCache {
  data: CoreTickerIncomeResponse;
  loadedKey: string;
}

export function useCoreStockIncome(ctx: CoreTickerContext | null) {
  ...
  const [data, setData] = useState<CoreTickerIncomeResponse | null>(() => {
