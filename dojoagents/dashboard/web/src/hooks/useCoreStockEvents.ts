...
import { fetchCoreTickerEvents } from '../api/dojoCore';
import { cacheKeys } from '../cache/cacheKeys';
import { fetchCached, getCached } from '../cache/queryCache';
import type { CoreTickerContext } from '../navigation/coreContext';
import type { CoreTickerEventsResponse } from '../types/dojoCore';

const DEFAULT_PAGE_SIZE = 20;

interface CoreStockEventsCache {
  data: CoreTickerEventsResponse;
  loadedKey: string;
}

export function useCoreStockEvents(ctx: CoreTickerContext | null, pageSize = DEFAULT_PAGE_SIZE) {
  const cacheKey =
  ...
  const [data, setData] = useState<CoreTickerEventsResponse | null>(() => {
