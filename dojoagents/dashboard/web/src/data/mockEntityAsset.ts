...
function buildKline(seedKey: string, basePrice: number, bars: number): EntityKlineBar[] {
  const rand = mulberry32(hashSeed(seedKey));
  const out: EntityKlineBar[] = [];
...
function buildPeBand(seedKey: string): EntityPeBandPoint[] {
  const rand = mulberry32(hashSeed(`${seedKey}-pe`));
  const out: EntityPeBandPoint[] = [];
  const mean = 22;
...
const TSM_SNAPSHOT: EntityAssetSnapshot = {
  ticker: 'TSM',
  market: 'us',
  name: {
    zh: '台湾积体电路制造股份有限公司',
...
function buildGenericSnapshot(ctx: EntityTickerContext): EntityAssetSnapshot {
...
export function getMockEntityAsset(ctx: EntityTickerContext): EntityAssetSnapshot {
  const ticker = normalizeTicker(ctx.ticker);
  if (ticker === 'TSM') {
    return {
      ...TSM_SNAPSHOT,
