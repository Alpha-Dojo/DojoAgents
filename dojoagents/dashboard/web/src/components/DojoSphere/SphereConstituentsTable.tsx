...
interface SphereConstituentsTableProps {
...
function ConstituentsMarketColumn({
  ...
  const sortLabel: Record<SortKey, string> = {
    market_cap: t('sphere.sortByMarketCap'),
    change_percent: t('sphere.sortByDayChange'),
    window_change_percent: t('sphere.sortByYearChange'),
    pe: t('sphere.sortByPe'),
    pb: t('sphere.sortByPb'),
  };
    ...
    <section className="sphere-constituents-col" aria-label={MARKET_CODE[market]}>
      ...
      <div className="sphere-table-wrap sphere-table-wrap--split">
        {loading && items.length === 0 ? (
          <p className="sphere-table-card__status">{t('sphere.loading')}</p>
        ) : null}
        <div className="sphere-table-head">
          <table className="sphere-table">
            ...
            <thead>
              <tr>
                <th>{t('sphere.colCode')}</th>
                <th>{t('sphere.colName')}</th>
                <th>{t('sphere.colPrice')}</th>
                {renderSortHeader('change_percent', t('sphere.colDay'))}
                {renderSortHeader('window_change_percent', t('sphere.colYear'))}
                ...
                return (
                  ...
                  <td>
                    <button
                      ...
                      title={t('core.openTicker')}
                      aria-label={`${t('core.openTicker')}: ${row.ticker}`}
...
export function SphereConstituentsTable({
  selection,
  scope,
  onNavigateTab,
}: SphereConstituentsTableProps) {
