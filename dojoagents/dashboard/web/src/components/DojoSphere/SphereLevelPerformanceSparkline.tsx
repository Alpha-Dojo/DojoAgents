...
interface SphereLevelPerformanceSparklineProps {
...
export function SphereLevelPerformanceSparkline({
  seriesByMarket,
  loading = false,
  hoverDate = null,
  onHoverDateChange,
}: SphereLevelPerformanceSparklineProps) {
  ...
  const marketTitle = useCallback(
    (chip: { date: string }, isClosed: boolean, isLatest: boolean) => {
      ...
      if (isClosed) parts.push(t('sphere.sessionClosed'));
      if (isLatest) parts.push(t('sphere.sessionLatest'));
      return parts.join(' · ');
    },
    [t],
  );
  ...
  return (
    <div className="sphere-level-sparkline">
      <div className="sphere-level-sparkline__head">
        <span className="sphere-level-sparkline__label">{t('sphere.performanceReturn')}</span>
        <div className="sphere-level-sparkline__head-tail">
          {displaySnapshot ? (
            ...
            <span className="sphere-level-sparkline__head-placeholder">{t('sphere.loadingPerformance')}</span>
          ) : null}
        </div>
      </div>
      <div
