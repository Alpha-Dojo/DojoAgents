...
export function CoreProfitabilityPanel({ axes, industryLabel, loading = false }: CoreProfitabilityPanelProps) {
    ...
    <CoreCard title={t('core.profitabilityTitle')} className="core-card--profitability">
      <div className="core-profitability">
          ...
          <svg viewBox="0 0 220 200" className="core-profitability__radar">
            ...
            {radar.labels.map((label) => (
              <text key={label.key} x={label.x} y={label.y} className="core-profitability__axis-label" textAnchor="middle">
                {t(`core.profit.${label.key}` as 'core.profit.grossMargin')}
              </text>
            ))}
          </svg>
        </div>
        ...
        <div className="core-profitability__ranks">
          <p className="core-profitability__ranks-title">{t('core.percentileInIndustry', { industry: industryLabel })}</p>
              ...
              return (
                <li key={axis.key} className="core-profitability__rank-item">
                  <div className="core-profitability__rank-head">
                    <span>{t(`core.profit.${axis.key}` as 'core.profit.grossMargin')}</span>
