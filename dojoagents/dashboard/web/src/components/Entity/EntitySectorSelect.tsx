...
interface EntitySectorSelectProps {
...
export function EntitySectorSelect({
  level,
  taxonomy,
  selection,
  onChange,
  onOpenSphereLevel,
}: EntitySectorSelectProps) {
  ...
  const levelLabelKey =
    level === 'L1' ? 'sectorPage.level1' : level === 'L2' ? 'sectorPage.level2' : 'sectorPage.level3';
  ...
  return (
    <div className="core-sector-field">
      <button
        ...
        aria-label={t('entityPage.openLevelInSphere', { level })}
        title={t('entityPage.openLevelInSphere', { level })}
