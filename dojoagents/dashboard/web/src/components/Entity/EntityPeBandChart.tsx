...
import { PERFORMANCE_MARKET_CLASS } from '../../utils/sectorPerformanceSeries';
...
interface EntityPeBandChartProps {
...
function buildAxisTicksFromDates(dates: string[]): Array<{ date: string; label: string; align: 'start' | 'center' | 'end' }> {
  if (!dates.length) return [];
  const normalized = dates.map((date) => formatKlineDate(date));
  ...
  const midIndex = Math.floor((normalized.length - 1) / 2);
  const midDate = normalized[midIndex] ?? startDate;
  const ticks = [
    { date: startDate, align: 'start' as const },
    ...(normalized.length > 2 ? [{ date: midDate, align: 'center' as const }] : []),
...
function buildPePath(
  visiblePoints: EntityPeBandPoint[],
  min: number,
  max: number,
  plotW: number,
  plotX0: number,
...
export function EntityPeBandChart({
  points,
  loading = false,
  sectorPeByMarket = {},
  ...
  lossLabel,
  lossOnly = false,
}: EntityPeBandChartProps) {
  const { t } = useTranslation();
  const resolvedLossLabel = lossLabel ?? t('valuation.peLoss');
  ...
  const isHovering = hover != null || externalDisplayPoint != null || (lossOnly && linkedHoverDate != null);
  ...
  const axisTicks = useMemo(() => {
    ...
    return buildAxisTicksFromDates(axisDates);
  }, [visiblePoints, axisDates]);

  const headerDate = displayPoint
    ? formatKlineDate(displayPoint.date)
    : lossOnly && linkedHoverDate
      ? formatKlineDate(linkedHoverDate)
      : anchorDate
        ? formatKlineDate(anchorDate)
        : null;
  const headerPeLabel = lossOnly || (displayPoint != null && displayPoint.pe < 0)
    ? resolvedLossLabel
    : displayPoint
      ? formatPeAxis(displayPoint.pe, resolvedLossLabel)
      : null;
  const showHeaderFields = Boolean(headerDate && headerPeLabel);
  const activePointDate = headerDate;
  ...
  if (loading && !points.length && !lossOnly) {
    return (
  ...
  if (!points.length && !lossOnly) {
    return (
      <EntityCard className="entity-card--pe-band">
        <div className="core-pe-band">
          <p className="entity-chart-stage__status">{t('entityPage.peBandEmpty')}</p>
        </div>
      </EntityCard>
    );
  }
    ...
    <EntityCard className="entity-card--pe-band">
      <div className={`entity-chart-shell${isHovering ? ' entity-chart-shell--hover' : ''}`}>
        <div className={`entity-kline__topbar${isHovering ? ' entity-kline__topbar--hover' : ''}`}>
          <span className="entity-kline__topbar-title">{t('entityPage.peBandTitle')}</span>
          {showHeaderFields ? (
            <div className="entity-kline__topbar-fields" aria-live="polite">
              <time
                className="entity-kline__field entity-kline__field--date"
                dateTime={headerDate ?? undefined}
              >
                <i>{t('entityPage.klineDate')}</i>
                <b>{headerDate}</b>
              </time>
              <span className="entity-kline__field">
                <i>{t('entityPage.peRatio')}</i>
                <b className="core-pe-band__val--pe">{headerPeLabel}</b>
              </span>
            </div>
          ) : null}
        </div>
        ...
        <div className="entity-chart-plots" ref={chartPlotsRef}>
              ...
              {axisTicks.map(({ date, label, align }) => (
                <span
                  ...
                  className={`entity-kline__time-tick entity-kline__time-tick--${align}${
                    isHovering && activePointDate === date ? ' entity-kline__time-tick--active' : ''
                  }`}
                >
                  {label}
                </span>
          ...
          <div className="entity-chart-strip-pane">
            <div className="core-pe-band__sector-pe" aria-label={t('entityPage.sectorPeTitle')}>
              <div className="core-pe-band__sector-level" role="tablist" aria-label={t('entityPage.sectorPeLevel')}>
              ...
              <div className="core-pe-band__sector-markets">
                {sectorPePanel.map((item) => (
                  ...
                  >
                    ...
                    <span className="core-pe-band__sector-pe-metric">{t('entityPage.peRatio')}</span>
