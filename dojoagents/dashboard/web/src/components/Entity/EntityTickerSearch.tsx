...
interface EntityTickerSearchProps {
  ticker: string;
  market: MarketCode;
  onSelect: (item: EntityTickerSearchItem) => void;
}
...
export function EntityTickerSearch({ ticker, market, onSelect }: EntityTickerSearchProps) {
  ...
  return (
    <div className="core-ticker-search" ref={rootRef}>
      ...
      <input
        ...
        value={query}
        placeholder={t('entityPage.tickerSearchPlaceholder')}
      ...
      {panelOpen ? (
        <div className="core-ticker-search__panel dojo-dropdown-select__dropdown">
          <ul id={listId} className="core-ticker-search__list" role="listbox">
            {loading ? <li className="core-ticker-search__status">{t('entityPage.searching')}</li> : null}
            {!loading && results.length === 0 ? (
              <li className="core-ticker-search__status">{t('entityPage.noSearchResults')}</li>
            ) : null}
            {!loading
              ? results.map((item) => (
