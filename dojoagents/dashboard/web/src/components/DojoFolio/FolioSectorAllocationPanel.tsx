...
import { openSphereFromSelection } from '../../navigation/openSphereSector';
...
export function FolioSectorLevelTabs({
  ...
  return (
    <div
      ...
      role="tablist"
      aria-label={t('core.sectorPeLevel')}
    >
      {SECTOR_LEVELS.map((level) => (
        ...
        >
          {t(level === 'L1' ? 'sphere.level1' : level === 'L2' ? 'sphere.level2' : 'sphere.level3')}
        </button>
      ))}
    </div>
  );
