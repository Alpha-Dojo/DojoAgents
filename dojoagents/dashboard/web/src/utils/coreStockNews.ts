import type { CoreStockNewsItem, StockNewsRow } from '../types/dojoCore';
...
export function mapStockNewsToCoreItems(rows: StockNewsRow[]): CoreStockNewsItem[] {
  return rows
    .map((row, index) => {
      const title = String(row.title ?? '').trim();
    ...
    .filter((item): item is CoreStockNewsItem => item != null);
}
