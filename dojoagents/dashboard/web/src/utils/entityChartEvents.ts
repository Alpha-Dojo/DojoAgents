import type { EntityChartEvent, EntityKlineBar, StockFinIndicatorRow } from '../types/entity';
import { extractReportDate } from './entityFinIndicators';
import { findClosestDateIndex, normalizeChartDates } from './entityChartLink';
import { candleSlot } from './entityCharts';

/** Supported K-line event kinds — extend with new types and CSS modifiers. */
export type EntityChartEventKind = EntityChartEvent['kind'];
...
export interface EntityChartEventMarker {
  event: EntityChartEvent;
  visibleIndex: number;
  cx: number;
}

export function mapEventsToVisibleMarkers(
  events: EntityChartEvent[],
  visibleBars: EntityKlineBar[],
  visibleStartIndex: number,
  allBars: EntityKlineBar[],
  plotW: number,
  plotX0: number,
): EntityChartEventMarker[] {
  ...
  const markers: EntityChartEventMarker[] = [];
