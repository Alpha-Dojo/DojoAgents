...
_BUILTIN_SCENES: tuple[tuple[str, VizStance, str, str], ...] = (
    (
        ...
        "组合写入/删改/下单/清仓属于交易确认类任务。用正文 markdown 汇报即可；"
        "可复用 portfolio_read_detail 自动附带的 viz_blocks，禁止再调 agent_viz_build。",
    ),
    (
        "portfolio_eval_accepted",
        "forbidden",
        "portfolio_eval_submit already accepted — task is closed. Reply to the user and stop; "
        "no extra visualization.",
        "portfolio_eval_submit 已通过，任务已闭环。直接回复用户，不要追加可视化。",
    ),
    (
        "quant_viz_data_ready",
        "encouraged",
        "Structured VIZ_DATA / viz_hint is available from execute_code. "
        "Call agent_viz_build with mapping_hint when a chart adds insight beyond stdout.",
        "execute_code 已产出 VIZ_DATA / viz_hint。需要图表时调用 agent_viz_build（带 mapping_hint）。",
    ),
    (
        "exploratory_read_analysis",
        "optional",
        "Read-only analysis: prefer viz_blocks auto-attached to data tools. "
        "Call agent_viz_build only when you need a chart type the auto blocks do not provide.",
        "只读分析：优先使用数据工具自动附带的 viz_blocks；"
        "仅当需要 auto 未提供的图表类型时才调用 agent_viz_build。",
    ),
    (
        "default_dashboard",
        "optional",
        "Default: markdown for narrative; agent_viz_build only when it clearly adds a chart "
        "the user asked for or auto viz_blocks cannot express.",
...
def _has_viz_hint_payload(result: ToolResult) -> bool:
    ...
    if isinstance(data, dict) and (
        (isinstance(data.get("dates"), list) and isinstance(data.get("prices"), list))
        or data.get("drawdown_pcts") is not None
    ):
        return True
    return False
...
def build_viz_policy_catalog(locale: str) -> str:
    ...
    if locale == "zh":
        ...
        intro = (
            ...
            "`dojoagents.agent.viz_policy.register_viz_policy_rule` 注册规则，勿改 prompt 散点补丁。\n\n"
        )
        ...
        intro = (
            ...
            "Register new scenes via `dojoagents.agent.viz_policy.register_viz_policy_rule` "
...
def build_viz_policy_turn_anchor(request: ChatRequest, locale: str) -> str:
    ...
    if any(marker.lower() in message.lower() for marker in transactional_markers):
        scene = _BUILTIN_SCENES[0]
        if locale == "zh":
            return (
                "## 本轮可视化策略\n"
                "检测到组合交易/写入类请求：任务完成后用 markdown 确认即可，"
                "**禁止** 调用 agent_viz_build。"
            )
        return (
            "## Visualization this turn\n"
            "Portfolio write/trade task detected: confirm in markdown when done; "
            "do NOT call agent_viz_build."
        )
