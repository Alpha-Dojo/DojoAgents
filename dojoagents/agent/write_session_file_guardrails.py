...
from dojoagents.agent.turn_intent import _format_history_for_classifier
from dojoagents.logging import LOGGER

WRITE_SESSION_FILE_TOOL_NAME = "write_session_file"

_CLASSIFIER_SYSTEM_PROMPT = (
    "You decide whether a finance agent may write a session output file on behalf of the user.\n"
    ...
    "- allow_write=false for routine analysis, screening, portfolio updates, charts, or tables "
...
def _turn_cache_key(user_message: str, history: list | None) -> str:
    history_block = _format_history_for_classifier(history or [], max_messages=4)
...
async def classify_write_session_file(
    ...
    *,
    ...
    history_block = _format_history_for_classifier(history or []) if history else "(no prior turns)"
    user_payload = (
