...
class ArtifactSynthesisHarness(TaskOutputHarnessMixin, TaskHarness):
    ...
    def matches(self, request: ChatRequest, state: HarnessLoopState) -> bool:
        ...
        return active is not None and (
            active.harness_profile == self.name or active.harness_profile == "artifact_synthesis"
        )
    ...
    def block_tool_call(self, call: ToolCall, state: HarnessLoopState) -> str | None:
        ...
        if call.name == "write_session_file" and active.constraints.get("must_read_input_before_write", True):
            ...
            if missing:
                return (
                    "Call read_session_output for required inputs before write_session_file: "
                    + ", ".join(missing)
                )
        return None
    ...
    def validate_progress(self, state: HarnessLoopState) -> HarnessDecision:
        ...
        if unread:
            ...
            issues.append(
                "Write the required JSONL output with write_session_file. "
                "A markdown attribution table in chat does not count as task completion."
            )
