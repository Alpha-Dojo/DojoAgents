...
from dojoagents.harnesses.built_in.financial.contracts.dojo_core import CoreTickerPeBandResponse
...
from dojoagents.harnesses.built_in.financial.services.domain_utils import (
    filter_date_rows,
    finite_float,
...
from dojoagents.harnesses.built_in.financial.services.fin_indicators_utils import report_type_for_market
from dojoagents.harnesses.built_in.financial.services.kline_bar_utils import DATA_START_DATE, resolve_tail_limit
from dojoagents.harnesses.built_in.financial.services.ticker_symbol_resolution import resolve_ticker_symbol
from dojoagents.harnesses.built_in.financial.services.market_sector_lead import (
    MAX_SECTOR_MEMBERS,
...
from dojoagents.harnesses.built_in.financial.services.sector_movers_ranking import sector_eligible_for_movers_ranking
from dojoagents.harnesses.built_in.financial.services.sector_leader_concentration import compute_leader_concentration
from dojoagents.harnesses.built_in.financial.services.market_stats import compute_market_stats
from dojoagents.harnesses.built_in.financial.services.portfolio_service import DEFAULT_BENCHMARKS
from dojoagents.harnesses.built_in.financial.services.sector_constituents import MARKETS
from dojoagents.harnesses.built_in.financial.services.sector_constituents_list import list_sector_constituents
from dojoagents.harnesses.built_in.financial.services.sector_scope_performance import (
    compute_sector_scope_performance,
)
from dojoagents.harnesses.built_in.financial.services.sector_scope_stats import compute_sector_scope_metrics
...
def _sector_path_id_format_error(path_id: str) -> str:
    ...
    return f"Invalid sector_path_id {path_id!r}. Expected format level1_id/level2_id/level3_id " "from search_sector_taxonomy."
...
def _sector_search_item(path: Any, *, hit: Any | None = None) -> dict[str, Any]:
    ...
    item: dict[str, Any] = {
        ...
        "scope_hint": (
            f"matched_level={matched_level}: use scope={constituent_scope!r} in filter_sector_constituents " "but always pass the full three sector ids (or sector_path_id)."
        ),
        "next_call": {
...
def resolve_sector_path(
    ...
    *,
    ...
    if path_id:
        ...
        _raise_sector_path_resolution_error(
            f"unknown sector_path_id: {path_id}. " "Call search_sector_taxonomy with the concept keyword and copy ids from best_match.",
            suggestions=suggestions,
        )
    ...
    if l1 and l2 and not l3:
        message += " For L2-scope constituents, still pass all three ids from best_match " "(or level1_id+level2_id when uniquely resolved) and set scope='L2'."
...
def build_taxonomy_tree(registry) -> dict[str, Any]:
    ...
    return {
        ...
        "usage": ("Copy level1_id, level2_id, level3_id from the SAME L3 branch below. " "These are opaque sector_id strings (e.g. 153/160/161), NOT array indices 1/2/3."),
        "playbook": {
...
async def build_portfolio_analysis_v1(
    ...
    *,
    ...
    if benchmark:
        ...
        from dojoagents.harnesses.built_in.financial.services.portfolio_candidate_index import parse_candidate_index_symbol
