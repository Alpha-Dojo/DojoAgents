...
3. For large prior tool outputs, pass the pointer's semantic selector through
   `execute_code.artifact_inputs`, then use `dojo_tools.input_result(name)`. Never copy a
   `call_id` into Python code. The artifact pointer includes `schema_hint` and `parse_hint`.
4. Parse tool payloads with `dojo_tools.tool_json(res)`; metadata scalars via
   ...
   Example:
   `res = dojo_tools.input_result('prices'); dojo_tools.tool_print(res, table='items')`
